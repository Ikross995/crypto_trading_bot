# apply_live_engine_fix_patch.py
# -*- coding: utf-8 -*-
...
# Runtime overrides — не валим модуль, если его нет
try:
    from infra.settings import RuntimeOverridesWatcher, apply_settings_to_config
except Exception:  # модуль опциональный
    RuntimeOverridesWatcher = None
    def apply_settings_to_config(cfg, changes):  # no-op
        return
...
class LiveTradingEngine:
    ...
    def __init__(self, config: Config):
        ...
        logger.info("Live trading engine initialized", symbol=config.symbol, mode="LIVE")
    ...
    async def _shutdown_components(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error shutting down components: {e}", exc_info=True)

    def _setup_signal_handlers(self) -> None:
        """Setup signal handlers for graceful shutdown."""

        def _handle(sig_num, frame):
            logger.info(f"Received signal {sig_num}, shutting down...")
            try:
                loop = asyncio.get_running_loop()
                loop.call_soon_threadsafe(asyncio.create_task, self.stop())
            except RuntimeError:
                # Нет активного цикла — сделаем синхронно
                try:
                    asyncio.run(self.stop())
                except RuntimeError:
                    # Цикл уже идёт в этом же потоке: просто ставим флаг
                    self._shutdown_event.set()
    ...
    async def _run_trading_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                # Runtime overrides (если файл правили на лету)
                ...
                # Calculate loop timing and sleep
    ...
    async def _process_trading_cycle(self) -> None:
        ...
        # 1) Signal
        signal = await self.signal_generator.generate_signal(self.config.symbol)
        ...
        # 4) DCA (optional)
        if bool(getattr(self.config, "dca_enabled", False)):
    ...
    async def _process_dca(self) -> None:
        ...
        for symbol in getattr(self.config, "symbols", [self.config.symbol]):
            try:
    ...
    async def _execute_dca(self, symbol: str, dca_action) -> None:
        """Execute a DCA action (stub)."""
        # Реализация зависит от вашей стратегии DCA
        return
    ...
    async def _can_trade_signal(self, signal: Signal) -> bool:
        ...
        # cooldown (если реализован асинхронно)
        try:
            if await self.order_manager.is_in_cooldown(signal.symbol):
                return False
        except TypeError:
            # если метод синхронный
            if self.order_manager.is_in_cooldown(signal.symbol):
                return False

        current_position = self.active_positions.get(signal.symbol)
        max_pos = float(getattr(self.config, "max_position_size", 1e9))
        if current_position and abs(getattr(current_position, "size", 0)) >= max_pos:
            return False
    ...
    def _is_trading_hours(self) -> bool:
        ...
        if not bool(getattr(self.config, "trading_hours_enabled", False)):
            return True
    ...
    async def _check_emergency_stop(self) -> bool:
        ...
        # 1) Daily loss: поддерживаем обе схемы — абсолют и % от баланса
        try:
            daily_pnl = await self.metrics.get_daily_pnl()
        except Exception:
            daily_pnl = 0.0

        mdl_abs = getattr(self.config, "max_daily_loss", None)
        mdl_pct = getattr(self.config, "max_daily_loss_pct", None)
        if mdl_abs is not None:
            if daily_pnl < -abs(float(mdl_abs)):
                logger.critical(f"Daily loss limit exceeded: {format_currency(daily_pnl)}")
                return True
        elif mdl_pct is not None:
            bal = 0.0
            try:
                bal = float(self.position_manager.get_account_balance() or 0.0)
            except Exception:
                bal = 0.0
            thresh = -abs(bal) * (float(mdl_pct)/100.0)
            if daily_pnl < thresh:
                logger.critical(f"Daily loss % limit exceeded: {format_currency(daily_pnl)} < {format_currency(thresh)}")
                return True

        # 2) Account balance floor
        min_bal = float(getattr(self.config, "min_account_balance", 0.0))
        try:
            balance = float(self.position_manager.get_account_balance() or 0.0)
        except Exception:
            balance = 0.0
        if min_bal > 0.0 and balance < min_bal:
            logger.critical(f"Account balance too low: {format_currency(balance)}")
            return True

        # 3) Max drawdown
        max_dd = getattr(self.config, "max_drawdown", None)
