# apply_cli_live_rescue_v5.py
# -*- coding: utf-8 -*-
"""
Единый rescue‑патч для быстрого запуска live/paper CLI без падений:
- чинит IndentationError в cli_integrated.py (перезапись корректной CLI)
- правит ошибку self.getattr(...) в runner/live.py
- добавляет validate_symbol() и мягкие аксессоры в core/utils.py
"""
...
def patch_live_py(path: Path):
    ...
    # на всякий случай почистим любые self.getattr(
    src = src.replace("self.getattr(", "getattr(")

    # 2) где напрямую дергают self.config.min_account_balance — завернем в getattr()
    src = re.sub(
...
def write_cli_integrated(path: Path):
    content = dedent("""\
    #!/usr/bin/env python3
    # -*- coding: utf-8 -*-
    \"\"\"
    ...
    def _build_config(mode: str,
        ...
        try:
            ...
            try:
                cfg = Config(**{k: v for k, v in cfg_kwargs.items() if k in Config.__fields__})
...
def main():
    ...
    if patch_live_py(live_path):
        print("✅ Patched runner/live.py (getattr and safe config access)")
        changed_any = True
    else:
        print("ℹ runner/live.py: no changes needed")

    # 3) core/utils.py: добавляем validate_symbol и помощники
    utils_path = BASE / "core" / "utils.py"
    if patch_utils_py(utils_path):
        print("✅ Patched core/utils.py (validate_symbol, csv_to_list, ensure_attr)")
        changed_any = True
    else:
        print("ℹ core/utils.py: helpers already present")

    print("\nSummary:")
    print("  Changed something:" if changed_any else "  Nothing changed")
    print("Done.")
