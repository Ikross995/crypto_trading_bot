# apply_live_engine_guardrails_v7.py
import os, sys, re, io, shutil, time
...
def patch_overrides_import(src: str) -> str:
    ...
    return SAFE_OVERRIDES_IMPORT + "\n" + src

def patch_live_class(src: str) -> str:
...
def main():
    ...
    # patch class
    src = patch_live_class(src)

    # write back
    with io.open(LIVE_PATH, "w", encoding="utf-8", newline="\n") as f:
        f.write(src)

    print("Patched runner/live.py -> backup:", os.path.basename(BACKUP_PATH))
